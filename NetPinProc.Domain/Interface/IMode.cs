...
namespace NetPinProc.Domain
{
    ...
    {
        /// <summary>Reference to the hosting GameController object / descendant</summary>
        IGameController Game { get; set; }

        /// <summary>The priority of the mode in the queue</summary>
        int Priority { get; set; }

        /// <summary>The display layer for the mode</summary>
        ILayer Layer { get; set; }

        /// <summary>Adds a switch handler to the AcceptedSwitch list for the mode</summary>
        ...
        /// <summary>Cancel a delayed event</summary>
        /// <param name="Name">The name of the delay to cancel</param>
        void CancelDelayed(string Name);

        /// <summary>Delay an event for the specified period of time</summary>
        ...
        /// <summary>Called by the game controller run loop during each loop when the mode is running</summary>
        void ModeTick();

        /// <summary>Notifies the mode that it is now the topmost mode on the mode queue <para/>
        /// This method should not be invoked directly, it is called by the GameController run loop</summary>
        void ModeTopMost();

        /// <summary>Type name and <see cref="Priority"/></summary>
        /// <returns></returns>
        string ToString();

        /// <summary>Called by the GameController to re-apply active lamp schedules</summary>
        void UpdateLamps();
    }
}