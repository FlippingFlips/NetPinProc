...
namespace NetPinProc.Domain
{
    ...
    public class MachineConfiguration
    {
        ...
        {
            var cfg = JsonSerializer.Deserialize<MachineConfiguration>(JSON, new JsonSerializerOptions()
        ...
        {
            return machineItemType switch
            {
                MachineItemType.Switch => PRSwitches?.Where(x => x.Tags != null && x.Tags.ToLower().Contains(tag.ToLower())).Select(n => n.Name)?.ToArray(),
                MachineItemType.Lamp => PRLamps?.Where(x => x.Tags != null && x.Tags.ToLower().Contains(tag.ToLower())).Select(n => n.Name)?.ToArray(),
                MachineItemType.Led => PRLeds?.Where(x => x.Tags != null && x.Tags.ToLower().Contains(tag.ToLower())).Select(n => n.Name)?.ToArray(),
                MachineItemType.Coil => PRCoils?.Where(x => x.Tags != null && x.Tags.ToLower().Contains(tag.ToLower())).Select(n => n.Name)?.ToArray(),
                MachineItemType.GI => PRGI?.Where(x => x.Tags != null && x.Tags.ToLower().Contains(tag.ToLower())).Select(n => n.Name)?.ToArray(),
                _ => null,
            };
        }
        ...
        {
            XmlSerializer serializer = new (typeof(MachineConfiguration));
