...
{
    /// <summary>Core object representing the game itself.</summary>
    public interface IGameController
    {
        /// <summary>The number of the current ball. A value of 1 represents the first ball; 0 indicates game over.</summary>
        int Ball { get; }

        /// <summary>All coils/solenoids within the game</summary>
        AttrCollection<ushort, string, IDriver> Coils { get; set; }

        /// <summary>Current machine configuration representation</summary>
        MachineConfiguration Config { get; }

        /// <summary>Current player playing</summary>
        int CurrentPlayerIndex { get; }

        /// <summary>Are the flippers enabled?</summary>
        bool FlippersEnabled { get; }

        /// <summary>All GI drivers within the game</summary>
        AttrCollection<ushort, string, IDriver> GI { get; set; }

        /// <summary>The lamp controller with lampshows</summary>
        ILampController LampController { get; set; }

        /// <summary>All lamps within the game</summary>
        AttrCollection<ushort, string, IDriver> Lamps { get; set; }

        /// <summary>All leds within the game</summary>
        ...
        /// <summary>The list of players currently playing the game</summary>
        ...
        /// <summary>All switches and optos within the game</summary>
        AttrCollection<ushort, string, Switch> Switches { get; set; }

        /// <summary>Invokes <see cref="CreatePlayer"/> to create default name with Player, then adds a new player to 'Players'</summary>
        /// <returns></returns>
        IPlayer AddPlayer();

        /// <summary>Adds points to the current player</summary>
        void AddPoints(long points);

        /// <summary>Adds bonus to the current player</summary>
        void AddBonus(long bonus);

        /// <summary>Called by the game framework when the current ball has ended</summary>
        void BallEnded();

        /// <summary>Called by the game framework when a new ball is starting</summary>
        void BallStarting();

        /// <summary>Creates a new player with a given name</summary>
        ...
        /// <summary>Returns the current 'Player' object according to the current_player_index value</summary>
        /// <returns></returns>
        IPlayer CurrentPlayer();

        /// <summary>Called by the GameController when a DMD event has been received.</summary>
        void DmdEvent();
        ...
        /// <summary>Called by the implementor to notify the game that the current ball has ended</summary>
        void EndBall();

        /// <summary>Called by the implementor to notify the game that the game as ended. Should call GameEnded</summary>
        void EndGame();

        /// <summary>Set the exit condition for the run loop causing the game to terminate</summary>
        void EndRunLoop();

        /// <summary>Called by the GameController when the current game has ended</summary>
        void GameEnded();

        /// <summary>Called by the GameController when a new game is starting. Creates new list of players</summary>
        void GameStarted();

        /// <summary>The ball time for the current player</summary>
        /// <returns>The ball time (in seconds) that the current ball has been in play</returns>
        double GetBallTime();

        /// <summary>Retrieve all events from the PROC interface board</summary>        
        ...
        /// <summary>The game time for the given player index</summary>
        ...
        /// <summary>Updates switch rules for flipper linked coils. Main flipper is pulsed and the hold with be pulsed with 0 <para/></summary>
        ...
        /// <summary>Create a new machine configuration representation in memory from a json file on disk.</summary>
        ...
        /// <summary>Reset the game state to normal (like a slam tilt). Clear players, layers and modes. </summary>
        void Reset();

        /// <summary>Should run a game loop, getting events, ticking drivers and modes</summary>
        void RunLoop(byte delay = 0, CancellationTokenSource cancellationToken = default);

        /// <summary>Safely disable a coil from another thread</summary>
        /// <param name="coilName">The coil name to disable</param>
        void SafeDisableCoil(string coilName);

        /// <summary>Safely drive a coil from another thread</summary>
        ...
        /// <summary>Save the ball start time into local memory</summary>
        void SaveBallStartTime();

        /// <summary>Called by the game framework when a new ball is starting which was the result of a stored extra ball. <para/>
        /// The default implementation calls <see cref="BallStarting"/> which is not called by the framework in this case.
        /// </summary>
        void ShootAgain();

        /// <summary>Called by the implementor to notify the game that the first ball should be started.</summary>
        void StartBall();

        /// <summary>Called by the implementor to notify the game that the game has started.</summary>
        void StartGame();

        /// <inheritdoc/>
        void Tick();

        /// <summary>Propagate Tick events to all lamps, coils and leds</summary>
        void TickVirtualDrivers();

        /// <summary>Should invoke UpdateLamps for every mode running</summary>
        void UpdateLamps();
    }
}